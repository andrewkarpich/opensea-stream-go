...
func main() {
	...
	client := opensea.NewStreamClient(
		types.MAINNET, "apikey", phx.NewSimpleLogger(phx.LogInfo), func(err error) {
	...
	client.OnItemListed(
		"collection-slug", func(response any) {
	...
	client.On(
		[]types.EventType{types.ItemListed, types.ItemTransferred}, "collection-slug", func(response any) {
			// ...
			fmt.Printf("%+v\n", response)
		},
	)
